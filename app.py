...
def home():
    ...
    if not nrorc:
        # Formulário com logo centralizado e dropdown filial
...
''')
    ...
    # Visualização HTML com total geral
    html_tpl = '''
<!DOCTYPE html><html lang="pt-br"><head><meta charset="UTF-8"><title>Orçamento {{order}}</title>
<style>/* estilos iguais anteriores */body{font-family:Arial,sans-serif;margin:20px}header,footer{background:#f0f0f0;padding:10px;overflow:hidden}header{display:flex;align-items:center}header img{height:100px}header .info{margin-left:auto;text-align:right}.clear{clear:both}.section{margin-top:20px}.section .header{background:rgb(200,230,200);color:#3C3C3C;padding:6px;font-weight:bold}.items div{display:flex;padding:6px 0}.items .descr{flex:1}.items .qty,.items .unit{width:50px;text-align:center}.volume-total{margin:10px 0;overflow:hidden}.volume-total .left{float:left}.volume-total .right{float:right}.total-geral{margin-top:20px;font-size:1.1em;font-weight:bold;text-align:right}a.btn{display:inline-block;margin-top:20px;padding:8px 12px;background:#189c00;color:#fff;text-decoration:none;border-radius:4px}footer{font-size:0.8em;color:#666;text-align:center;margin-top:40px}</style></head><body>
<header><img src="/logo.png" alt="Logo"><div class="info"><div><strong>ORÇAMENTO:</strong> {{order}}-{{total_forms}}</div>{% if patient %}<div><strong>PACIENTE:</strong> {{patient}}</div>{% endif %}</div><div class="clear"></div></header>
<main>{% for info in grouped.values() %}<div class="section"><div class="header">Formulação {{"%02d"|format(loop.index)}}</div><div class="items">{% for it in info['items'] %}<div><span class="descr">{{it.descr}}</span><span class="qty">{{it.quant}}</span><span class="unit">{{it.unida}}</span></div>{% endfor %}</div><div class="volume-total"><div class="left"><strong>Volume:</strong> {{info.volume}} {{info.univol}}</div><div class="right"><strong>Total:</strong> R$ {{"%.2f"|format(info.prcobr)}}</div><div class="clear"></div></div></div>{% endfor %}</main>
