...
class PDF(FPDF):
    def header(self):
        ...
        if os.path.exists(logo_path):
            try:
                self.image(logo_path, x=10, y=2, w=100)
            except:
                pass
...
# Carrega itens agrupados e dtentr
def load_grouped(sql):
    ...
    # captura data de entrada como date
...
<!DOCTYPE html>
<html lang="pt-br">
<head><meta charset="UTF-8"><title>Consultar Orçamento</title>...form css...</head>
<body>...form html...</body>
</html>
''')
    # consulta itens + dtentr
    sql = (
    ...
    html_tpl = '''...'''  # manter seu HTML, adicionando ao final:
    # após totais finais adicione:
    # <p><strong>Data do Orçamento:</strong> {{dtentr}}</p>
    # <p><strong>Validade do Orçamento:</strong> {{validade}}</p>
...
def generate_pdf():
    ...
    # datas do orçamento
    pdf.set_font('Arial', '', 11)
    pdf.cell(0, 8, f"Data do Orçamento: {dtentr}", ln=True)
    if validade:
        pdf.cell(0, 8, f"Validade do Orçamento: {validade}", ln=True)
